...
# stack frame layout in _x86_Karm_* routines, frame is allocated
# by caller
...
# stack frame layout in Karm_[en|crypt] routines, which differs from
...
&static_label("Karm_SIGMA");
&static_label("Karm_SBOX");
...
#		Byte ciphertext[])
&function_begin("Karm_EncryptBlock_Rounds");
	...
	&set_label("pic_point");
	&blindpop($Tbl);
	&lea	($Tbl,&DWP(&label("Karm_SBOX")."-".&label("pic_point"),$Tbl));
	...
	&mov	(&DWP(8,$idx),@T[2]);
	&mov	(&DWP(12,$idx),@T[3]);
&function_end("Karm_EncryptBlock_Rounds");
# V1.x API
&function_begin_B("Karm_EncryptBlock");
	&mov	("eax",128);
	&sub	("eax",&wparam(0));	# load keyBitLength
	&mov	("eax",3);
	&adc	("eax",0);		# keyBitLength==128?3:4
	&mov	(&wparam(0),"eax");
	&jmp	(&label("Karm_EncryptBlock_Rounds"));
&function_end_B("Karm_EncryptBlock");
...
&function_begin("Karm_encrypt");
	...
	&mov	(&DWP(8,$idx),@T[2]);
	&mov	(&DWP(12,$idx),@T[3]);
&function_end("Karm_encrypt");
}

&function_begin_B("_x86_Karm_encrypt");
...
&set_label("loop",16);
	for ($i=0;$i<6;$i++) { Karm_Feistel($i,16,4); }
...
&set_label("done",8);
	...
	&xor	(@T[3],&DWP(12,$key));
	&ret	();
&function_end_B("_x86_Karm_encrypt");

# void Karm_DecryptBlock_Rounds(
#		int grandRounds,
#		const Byte ciphertext[],
#		const KEY_TABLE_TYPE keyTable,
#		Byte plaintext[])
&function_begin("Karm_DecryptBlock_Rounds");
	&mov	("eax",&wparam(0));	# load grandRounds
	&mov	($idx,&wparam(1));	# load ciphertext pointer
	&mov	($key,&wparam(2));	# load key schedule pointer
	...
	&mov	(&DWP(8,$idx),@T[2]);
	&mov	(&DWP(12,$idx),@T[3]);
&function_end("Karm_DecryptBlock_Rounds");
# V1.x API
&function_begin_B("Karm_DecryptBlock");
	&mov	("eax",128);
	&sub	("eax",&wparam(0));	# load keyBitLength
	&mov	("eax",3);
	&adc	("eax",0);		# keyBitLength==128?3:4
	&mov	(&wparam(0),"eax");
	&jmp	(&label("Karm_DecryptBlock_Rounds"));
&function_end_B("Karm_DecryptBlock");
...
&function_begin("Karm_decrypt");
	...
	&mov	(&DWP(8,$idx),@T[2]);
	&mov	(&DWP(12,$idx),@T[3]);
&function_end("Karm_decrypt");
}

&function_begin_B("_x86_Karm_decrypt");
	&xor	(@T[0],&DWP(0,$key));	# ^=key[0-3]
	&xor	(@T[1],&DWP(4,$key));
	&xor	(@T[2],&DWP(8,$key));
	&xor	(@T[3],&DWP(12,$key));
...
&set_label("loop",16);
	for ($i=0;$i<6;$i++) { Karm_Feistel($i,-8,4); }
...
&set_label("done",8);
	...
	&xor	(@T[1],&DWP(4,$key));
	&ret	();
&function_end_B("_x86_Karm_decrypt");
...
&set_label("1st128",4);
	...
	&blindpop($Tbl);
	&lea	($Tbl,&DWP(&label("Karm_SBOX")."-".&label("pic_point"),$Tbl));
	&lea	($key,&DWP(&label("Karm_SIGMA")."-".&label("Karm_SBOX"),$Tbl));
...
&function_end("Karm_Ekeygen");

if ($OPENSSL) {
# int private_Karm_set_key (
#		const unsigned char *userKey,
#		int bits,
#		KARM_KEY *key)
&function_begin_B("private_Karm_set_key");
	&push	("ebx");
...
&set_label("done",4);
	&pop	("ebx");
	&ret	();
&function_end_B("private_Karm_set_key");
}
...
&function_begin("Karm_cbc_encrypt");
...
&set_label("DECRYPT",16);
    ...
    &mov	("esp",$_esp);
    &popf	();
&function_end("Karm_cbc_encrypt");
}

&asciz("Karm for x86 by <appro\@openssl.org>");
